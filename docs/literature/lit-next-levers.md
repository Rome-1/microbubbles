...
| 2 | **No-GT arbiter: split-half FRC + saturation + elevation-occupancy** (agreed #10b, unbuilt) | Odd/even-acq FRC (½-bit & 2σ), exponential saturation `C₀(1−e^{−κt})`, per-elev-plane occupancy + midplane-attraction audit, added to `bench.py` | n/a — adds metrics | **De-risks every other lever**: the only proxy that separates *recovered vessel* from *manufactured/over-merged track* (the stitch straightness regression and any coverage lever need this) | M | No (CPU on `.bin`) | Hingot 2021 FRC (verified-abstract); Hingot 2019 saturation (verified-abstract); G24 (verified-fulltext) |
